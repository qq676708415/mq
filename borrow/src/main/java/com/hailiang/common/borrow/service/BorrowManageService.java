...
/**
 * 债券匹配 消息队列监听，将队列中取到的值放入阻塞队列里
 *
 */
@Service("borrowManageService")
@Slf4j
public class BorrowManageService {
	 ...
	 * @throws Exception
	 */
	public void ListenBorrowMessages() {
		log.info("开始监听...");
		for (;;) {
			//  债券匹配预处理 - 队列监控的key
			String monitor_key = MqMonitorConst.FF_MQ_MONITOR + MqMonitorConst.MQ_FW_ASSETS_MANAGE_BORROW + Thread.currentThread().getId();
			...
			try {
				...
				String advanceValue = queueBiz.rightPop(RedisQueueConst.REDIS_QUEUE_BORROW_MANAGE_LIST, String.class);
				if (null == advanceValue) {
					continue;
				}
				log.info("债券匹配预处理队列出队 => {}", advanceValue);

				log.info("消息备份添加...");
				redisBatchBiz.hset(RedisQueueConst.REDIS_QUEUE_BORROW_MANAGE_BACKUP_MAP_KEY, advanceValue, advanceValue);
